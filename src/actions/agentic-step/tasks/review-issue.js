...
export async function reviewIssue(context) {
  ...
  if (!targetIssueNumber) {
    const { data: openIssues } = await octokit.rest.issues.listForRepo({
      ...repo, state: 'open', labels: 'automated', per_page: 1, sort: 'created', direction: 'asc',
    });
  ...
  const { data: issue } = await octokit.rest.issues.get({
    ...repo, issue_number: Number(targetIssueNumber),
  });
  ...
  const testFiles = scanDirectory(
    config.paths.targetTestsPath?.path || 'tests/', ['.test.js', '.test.ts'],
    { contentLimit: 2000, recursive: true }
  );
  ...
  if (verdict.toUpperCase().startsWith('RESOLVED')) {
    ...
    await octokit.rest.issues.update({
      ...repo, issue_number: Number(targetIssueNumber), state: 'closed',
    });
