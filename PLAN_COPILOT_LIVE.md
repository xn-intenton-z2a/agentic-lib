...
- **[FEATURES.md](FEATURES.md)** — Complete inventory of 17 features across all 3 repos, each with an Option E implementation mapping
...
- **Custom agents**: `.github/agents/*.md` — specialized prompt files
...
| Issue enhancement (wfr-completion-enhance-issue) | Custom agent via `.github/agents/enhancer.md` |
...
- Agent config files (repurposed as `.github/agents/*.md`)
...
- Custom agents (`.github/agents/*.md`) replace agent prompt files naturally
...
**Effort:** Low
- Delete ~40 workflows
- Configure `.github/agents/*.md` with good prompts
...
- Add `copilot-setup-steps.yml` and `.github/agents/*.md`
...
**Start with Option D (Hybrid)**, then evaluate migration to **Option E (SDK core)** once the Copilot SDK reaches GA.

### Phase 1: Hybrid (Option D) — Immediate

1. Add `copilot-setup-steps.yml` to both agentic-lib and repository0
2. Create `.github/agents/*.md` files mapping to existing agent prompts
3. Modify `agent-transformation-issue-to-code.yml` to assign issue to Copilot instead of calling OpenAI
4. Modify `agent-flow-fix-code.yml` to use `@copilot` on the PR
5. Update `ci-automerge.yml` to handle `copilot/*` branches
6. Keep OpenAI for: discussions bot, issue enhancement, feature maintenance, library maintenance
7. Measure: quality of Copilot PRs vs OpenAI PRs, time to merge, cost savings

### Phase 2: Expand Copilot (Option D+) — After validation

8. Migrate discussions bot to Copilot CLI (`copilot -p`)
9. Migrate issue enhancement to Copilot custom agent
10. Migrate feature/library maintenance to scheduled Copilot CLI
11. Remove OpenAI dependency entirely
12. Update stats to track Copilot premium requests instead of OpenAI tokens

### Phase 3: SDK Rewrite (Option E) — When Copilot SDK is GA

13. Build `agentic-step` GitHub Action using Copilot SDK
14. Simplify all flow/transformation workflows to use the action
15. Migrate discussions bot to use `agentic-step` with Copilot SDK
16. Publish the action to GitHub Marketplace
17. This delivers the "Move JS Steps to a GitHub distributed Action" and "Convert the actions library JS to an SDK" TODO items
...
- The "What is your intention?" submission box feeds through Giscus → GitHub Discussions → discussions bot
...
Which option to pursue, or which combination/phasing to adopt. See [FEATURES.md](FEATURES.md) for the detailed feature-by-feature Option E implementation mapping covering all 17 capabilities across all 3 repositories.
