...
export async function digestLambdaHandler(sqsEvent) {
  ...
  for (const [index, sqsEventRecord] of sqsEventRecords.entries()) {
    try {
      ...
      const recordId =
        sqsEventRecord.messageId || `fallback-${index}-${Date.now()}-${Math.random()
          .toString(36)
          .substr(2, 9)}`;
...
export async function main(args = process.argv.slice(2)) {
  ...
  if (await processVersion(filteredArgs)) {
    if (statsMode) {
      console.log(JSON.stringify({ callCount: globalThis.callCount, uptime: process.uptime() }));
    }
    return;
  ...
  if (await processDigest(filteredArgs)) {
    if (statsMode) {
      console.log(JSON.stringify({ callCount: globalThis.callCount, uptime: process.uptime() }));
    }
    return;
