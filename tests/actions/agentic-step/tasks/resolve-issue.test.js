...
import { runCopilotTask, readOptionalFile, formatPathsSection } from '../../../../src/actions/agentic-step/copilot.js';
...
describe('tasks/resolve-issue', () => {
  ...
  it('calls safety checks in correct order with correct parameters', async () => {
    ...
    expect(checkAttemptLimit).toHaveBeenCalledWith(
      ctx.octokit, ctx.repo, '42', 'agentic-lib-issue-', 2
    );
    expect(checkWipLimit).toHaveBeenCalledWith(
      ctx.octokit, ctx.repo, 'in-progress', 2
    );
  });
});
