...
describe("agenticHandler Single Command Processing", () => {
  ...
  test("throws an error and logs when command is non-actionable (NaN)", async () => {
    await expect(agenticLib.agenticHandler({ command: "NaN" })).rejects.toThrow("Invalid input: command is non-actionable (equivalent to 'NaN'). Please provide a valid, non-empty string.");
  });

  test("throws an error and logs when command is an empty string", async () => {
    await expect(agenticLib.agenticHandler({ command: "" })).rejects.toThrow("Invalid input: command is non-actionable (equivalent to 'NaN'). Please provide a valid, non-empty string.");
  });
...
describe("agenticHandler Batch Processing", () => {
  ...
  test("throws an error when one of the commands is invalid", async () => {
    ...
    await expect(agenticLib.agenticHandler(payload)).rejects.toThrow("Invalid input in commands: each command must be a valid, non-empty string (cannot be 'NaN').");
  });
