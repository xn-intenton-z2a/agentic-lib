...
describe("CLI CLI Utils Flag", () => {
  test("prints CLI utility commands summary when --cli-utils flag is provided", async () => {
    ...
    expect(output).toContain("--digest: Run a full bucket replay simulating an SQS event.");
    expect(output).toContain("--agentic <jsonPayload>: Process an agentic command using a JSON payload. Payload may include a 'command' or a 'commands' array for batch processing.");
    expect(output).toContain("--version: Show version information with current timestamp.");
    expect(output).toContain("--verbose: Enable detailed logging for debugging.");
    expect(output).toContain("--diagnostics: Output detailed diagnostics including configuration and environment details.");
    expect(output).toContain("--status: Display runtime health summary in JSON format.");
    expect(output).toContain("--dry-run: Execute a dry run with no side effects.");
    expect(output).toContain("--simulate-error: Simulate an error for testing purposes and exit.");
    expect(output).toContain("--simulate-delay <ms>: Simulate processing delay for the specified duration in milliseconds.");
    expect(output).toContain("--apply-fix: Apply automated fix and log success message");
    expect(output).toContain("--cli-utils: Display a summary of available CLI commands with their descriptions.");
    consoleSpy.mockRestore();
  });
});
